...
#### b) Can create .gff3 files for predicted gene structures and .cds predicted coding sequences for your de novo transcriptome
...
Mapped only one adult ambient sample (ACR5) first to see gene count output.
...
--transcripts /data/putnamlab/dbecks/Heatwave_A.pul_2022Project/data/trinity/Apul_denovo.fasta \
--seqType fq \
--SS_lib_type RF \
--prep_reference \
...
#completed after 8 hours 40 min
#see low gene counts per isoform, looking into this further but want to start Deseq2 analyses for all samples, so running all samples in this pipeline below
```
...
Mapping all 12 samples in seperate scripts to see gene count output.

```
nano /data/putnamlab/dbecks/Heatwave_A.pul_2022Project/scripts/trinity_map_all.sh
...
--SS_lib_type RF \
--prep_reference \
--left $(ls -m /data/putnamlab/dbecks/Heatwave_A.pul_2022Project/data/trimmed/*"R1_001.fastq.gz" | tr -d '[[:space:]]') \
--right $(ls -m /data/putnamlab/dbecks/Heatwave_A.pul_2022Project/data/trimmed/*"R2_001.fastq.gz" | tr -d '[[:space:]]') \
--est_method RSEM \
--aln_method bowtie2 \
--gene_trans_map /data/putnamlab/hputnam/Moorea_Parental/Trinity/Apul_denovo.fasta.gene_trans_map \
--thread_count $SLURM_CPUS_ON_NODE \
--output_dir /data/putnamlab/dbecks/Heatwave_A.pul_2022Project/data/trinity/mapped/all_samples


sbatch /data/putnamlab/dbecks/Heatwave_A.pul_2022Project/scripts/trinity_map_all.sh
...
```
nano /data/putnamlab/dbecks/Heatwave_A.pul_2022Project/scripts/gene_count_matrix.sh
...
#SBATCH --mem=32GB --cpus-per-task=24
#SBATCH --account=putnamlab
#SBATCH -D /data/putnamlab/dbecks/Heatwave_A.pul_2022Project/data/trinity/mapped/all_samples
...
sbatch /data/putnamlab/dbecks/Heatwave_A.pul_2022Project/scripts/gene_count_matrix.sh
```
