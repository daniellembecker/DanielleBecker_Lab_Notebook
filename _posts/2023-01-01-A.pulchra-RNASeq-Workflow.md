...
```
scp -r danielle_becker@ssh3.hac.uri.edu:/data/putnamlab/dbecks/Heatwave_A.pul_2022Project/data/mapped/results/mapped_reads_counts_Apul /Users/Danielle/Desktop/Putnam_Lab/A.pul_Heatwave/bioinformatics/data/mapping_percentages
...
We include a script to faciliate running of RSEM on Trinity transcript assemblies. The script we execute below will run the bowtie2 aligner to align reads to the Trinity transcripts, and RSEM will then evaluate those alignments to estimate expression values. Again, we need to run this separately for each sample and biological replicate (ie. each pair of fastq files).
...
#### a) Use Trinity for transcript quantification for estimating transcript abundance in a genome-free manner
...
# Paths
TRANSCRIPTOME="/data/putnamlab/dbecks/DeNovo_transcriptome/2023_A.pul/output/final_assembly/CD-HIT/stranded_assembly_cdhit.fasta"
TRINITY_PATH="/opt/software/Trinity/2.15.1-foss-2022a/trinityrnaseq-v2.15.1/util"

# Run Trinity for Reference Preparation
perl $TRINITY_PATH/align_and_estimate_abundance.pl \
...
sbatch /data/putnamlab/dbecks/Heatwave_A.pul_2022Project/scripts/prep_reference.sh


Submitted batch job 334886
```
...
TRANSCRIPT="/data/putnamlab/dbecks/DeNovo_transcriptome/2023_A.pul/output/final_assembly/CD-HIT/stranded_assembly_cdhit.fasta"
...
#### b) Build Transcript and Gene Expression Matrices
...
```
nano /data/putnamlab/dbecks/Heatwave_A.pul_2022Project/scripts/build_isoform_matrix.sh
...
perl $TRINITY_PATH/abundance_estimates_to_matrix.pl --est_method RSEM \
--gene_trans_map /data/putnamlab/dbecks/DeNovo_transcriptome/2023_A.pul/output/final_assembly/CD-HIT/stranded_assembly_cdhit.fasta.gene_trans_map \
--name_sample_by_basedir \
5_gene_count/RSEM.isoforms.results 8_gene_count/RSEM.isoforms.results 12_gene_count/RSEM.isoforms.results 16_gene_count/RSEM.isoforms.results 20_gene_count/RSEM.isoforms.results 24_gene_count/RSEM.isoforms.results 248_gene_count/RSEM.isoforms.results 249_gene_count/RSEM.isoforms.results 250_gene_count/RSEM.isoforms.results 260_gene_count/RSEM.isoforms.results 261_gene_count/RSEM.isoforms.results 262_gene_count/RSEM.isoforms.results

sbatch /data/putnamlab/dbecks/Heatwave_A.pul_2022Project/scripts/build_isoform_matrix.sh

Submitted batch job 334871

```


#### c) Secure-copy gene counts onto local computer, make sure to open a separate command shell outside of Andromeda on your own terminal

```
#copy gene count matrix

scp -r danielle_becker@ssh3.hac.uri.edu:/data/putnamlab/dbecks/Heatwave_A.pul_2022Project/data/trinity/trinity_mapped/all_samples/RSEM.gene.counts.matrix /Users/Danielle/Desktop/Putnam_Lab/A.pul_Heatwave/bioinformatics/data


#copy transcript count matrix
scp -r danielle_becker@ssh3.hac.uri.edu:/data/putnamlab/dbecks/Heatwave_A.pul_2022Project/data/trinity/trinity_mapped/all_samples/RSEM.isoform.counts.matrix /Users/Danielle/Desktop/Putnam_Lab/A.pul_Heatwave/bioinformatics/data
