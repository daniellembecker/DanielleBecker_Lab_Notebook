...
b) Convert .gff3 file to .gtf file for stringTie using [gffread](https://github.com/gpertea/gffread) from the [Cufflinks tools](https://ccb.jhu.edu/software/stringtie/gff.shtml#gffread) package on Andromeda
...
gffread stranded_assembly_cdhit.fasta.transdecoder.gff3 -T -o stranded_assembly_cdhit.gtf
```

c) Make directory for stringTie
...
d) Copy BAM files to stringTie folder

```
cd /data/putnamlab/dbecks/Heatwave_A.pul_2022Project/data/hisat2_mapped/results/
mv *_sorted.bam /data/putnamlab/dbecks/Heatwave_A.pul_2022Project/data/stringTie/BAM
```

e) Assemble and estimate reads
...
array1=($(ls $F/*bam))
for i in ${array1[@]}; do
	stringtie -G /data/putnamlab/dbecks/Heatwave_A.pul_2022Project/data/trinity/stranded_assembly_cdhit_cleaned.gtf -e -o ${i}.gtf ${i}
	echo "${i}"
done

sbatch stringTie_assemble.sh

Submitted batch job 335183
```
...
stringtie --merge -p 8 -G /data/putnamlab/dbecks/Heatwave_A.pul_2022Project/data/trinity/stranded_assembly_cdhit_cleaned.gtf -o stringtie_apul_merged.gtf apul_mergelist.txt
```
