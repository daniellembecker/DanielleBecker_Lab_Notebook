...
I was getting errors for downloading this step, but I found when I deleted the previous error and output file in the shared folder labeled output.4294967294.out and error.4294967294.out, the scripts run. Will update this output and error syntax so we don't run into the error again.
...
#SBATCH --exclusive
#SBATCH -D /data/putnamlab/dbecks/Heatwave_A.pul_2022Project/functional_annotation/blast

echo "START" $(date)
module load DIAMOND/2.1.8-GCC-12.3.0 #Load DIAMOND

echo "Updating apul annotation" $(date)
diamond blastp -b 2 -d /data/putnamlab/dbecks/Heatwave_A.pul_2022Project/data/trinity/trinity_out_dir.Trinity.fasta.transdecoder.pep -o apul_blastp_annot -f 100 -e 0.00001 -k 1 --threads $SLURM_CPUS_ON_NODE --tmpdir /tmp/
...
scp danielle_becker@ssh3.hac.uri.edu:/data/putnamlab/dbecks/Heatwave_A.pul_2022Project/functional_annotation/blast/apul_blastp_annot.xml /Users/Danielle/Desktop/Putnam_Lab/A.pul_Heatwave/bioinformatics/functional_annotation/diamond
```
...
```
cp Apul_proteins_names_v1.0.faa /data/putnamlab/dbecks/Becker_E5/Becker_RNASeq/BLAST-GO-KO/InterProScan/
sed -i 's/*//g' Apul_proteins_names_v1.0.faa
```
